...
# Function to mark attendance in
def mark_your_attendance(request):
    ...
    fa = face_utils.FaceAligner(predictor, desiredFaceWidth=96)
    ...
    while True:
        ...
        for face in faces:
            ...
            if pred != [-1]:
                ...
                messages.success(request, f"Welcome, {person_name}!")
                vs.stop()
                cv2.destroyAllWindows()
                update_attendance_in_db_in({person_name: True})
...
# Function to mark attendance out
def mark_your_attendance_out(request):
    ...
    while True:
        ...
        for face in faces:
            ...
            if pred != [-1]:
                person_name = encoder.inverse_transform(np.ravel([pred]))[0]
                messages.success(request, f"Goodbye, {person_name}!")
                vs.stop()
                cv2.destroyAllWindows()
                update_attendance_in_db_out({person_name: True})
                return redirect('home')
